...
export async function getAdminOrderSummaries(filters = {}, options = {}) {
	const { page, skip, take } = getOrderListQueryWindow(options.page);
	if (filters.invalidStatus) return toOrderListPageResult([], page);
	...
	return toOrderListPageResult(orders, page, toAdminOrderSummary);
}
