...
export async function requestPasswordReset(email, options = {}) {
  ...
  try {
    ...
    auditPasswordResetEvent("password_reset_requested", {
      ...
      rateLimitPrepared: sendRiskResult.meta?.rateLimitReady ?? false,
    });
...
export async function validatePasswordResetToken(email, token) {
  ...
  try {
    ...
    }, tokenRiskResult.meta ?? getPasswordResetTokenFeedbackMeta());
  } catch (error) {
...
export async function resetPasswordWithToken({ email, token, password }) {
  ...
  try {
    const validationResult = await validatePasswordResetToken(email, token);
