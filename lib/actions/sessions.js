...
import { getSessionByToken, listSessionsByUser, revokeSession } from "@/lib/server/sesion";
import { toActiveSessionView } from "@/lib/types";
import { getAuthenticatedSession, getSessionToken } from "@/lib/server/auth-session";
...
export async function getActiveSessions() {
  ...
  try {
    ...
    if (sessionToken) {
      const currentSessionResult = await getSessionByToken(sessionToken);
      if (currentSessionResult.ok) {
        currentSessionId = currentSessionResult.data.id;
      }
    }
...
export async function revokeActiveSession(sessionId) {
  try {
    ...
    return revokeSession(sessionId, sessionResult.data.id);
  } catch (error) {
