...
import { validateEmail, validatePassword, logError, logInfo, logWarn } from "@/lib/server/shared";
import { authenticateUserWithCredentials } from "@/lib/server/auth";
import { getUsuarioByEmailForAuth } from "@/lib/server/user/usuario";
  ...
  callbacks: {
    ...
    async session({ session, user }) {
      ...
      session.user = {
        ...
        emailVerificado: user?.emailVerificado ?? session?.user?.emailVerificado ?? false,
      };
      ...
      if (account?.provider === "google") {
        ...
        if (existingUserResult.ok) {
          ...
          if (existingUser.passwordHash) {
            logInfo(
              "signIn: Google con email existente por credenciales, se permite vinculacion segura",
              {
              action: "signIn",
              provider: account.provider,
          ...
          logInfo("signIn: Google con cuenta existente vinculada", {
            action: "signIn",
            provider: account.provider,
            userId: existingUser.id,
          });
        } else {
          logInfo("signIn: Google creara cuenta nueva automaticamente", {
            action: "signIn",
            provider: account.provider,
