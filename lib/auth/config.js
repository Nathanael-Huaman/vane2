...
const providers = [
  Credentials({
    ...
    async authorize(credentials) {
      ...
      if (!userResult.ok) {
        logWarn("authorize: usuario no encontrado o no disponible para auth", {
          action: "authorize",
          email,
      ...
      logInfo("authorize: usuario encontrado para auth", {
        action: "authorize",
        email,
        userId: user.id,
      ...
      if (!isValid) {
        logWarn("authorize: contrasena incorrecta", {
          action: "authorize",
          email,
          userId: user.id,
        });
