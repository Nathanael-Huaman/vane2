...
async function submitRegistrationForm(
	...
	const emailInput = page.getByLabel("Correo electronico");
	const passwordInput = page.getByLabel("Contrasena", { exact: true });
...
test.describe("Ticket 27 - Registro y navegacion (E2E funcional)", () => {
	...
	test("registro exitoso con credenciales redirige a confirmacion y persiste rol cliente", async ({
		...
		await expect(page).toHaveURL(
			new RegExp("/registro/confirmacion\\?email="),
			{
				timeout: 20_000,
			},
		);
	...
	test("registro duplicado muestra mensaje generico y no redirige a confirmacion", async ({
		...
		await expect(
			page.getByText("Ya existe una cuenta con ese correo electronico."),
		).toBeVisible();
		await expect(page).toHaveURL(/\/registro$/, { timeout: 10_000 });
	});

	// Spec: Flujo esperado > validaciones frontend antes de backend
	test("validaciones cliente bloquean submit con datos invalidos", async ({
		page,
	}) => {
		let requestCount = 0;
		...
		await expect(
			page.getByText("El formato del correo no es valido"),
		).toBeVisible();
		await expect(
	...
	test("boton registrarse con google inicia redireccion OAuth", async ({
		page,
	}) => {
		await page.goto("/registro");
	...
	test("verificacion con token valido muestra exito y limpia token", async ({
		page,
	}) => {
		...
		await page.goto(
			`/verificar-email?token=${encodeURIComponent(rawToken)}&email=${encodeURIComponent(email)}`,
		);
		await expect(
			page.getByText("Correo verificado", { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText("Correo verificado correctamente."),
		).toBeVisible();
		await expect(
	...
	test("verificacion con token invalido muestra error y opcion de reenvio", async ({
		...
		await expect(
			page.getByText("El enlace puede ser invalido o haber expirado."),
		).toBeVisible();
		await expect(
			page.getByRole("button", { name: "Reenviar correo de verificacion" }),
		).toBeVisible();
	});

	// Spec: Criterios de aceptacion > Navbar visible en todas las paginas
	test("navbar global visible en rutas publicas principales", async ({
		page,
	}) => {
