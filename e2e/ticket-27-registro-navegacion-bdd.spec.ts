...
test.describe("Ticket 27 - Registro y navegacion (BDD)", () => {
	...
	test("Given visitante en /registro When usa email existente Then muestra error generico", async ({
		...
		const email = uniqueEmail("bdddup");
	...
	test("Given token valido When verifica correo Then muestra exito", async ({
		page,
	}) => {
		...
		await page.goto(
			`/verificar-email?token=${rawToken}&email=${encodeURIComponent(email)}`,
		);
		await expect(
	...
	test("Given movil When render navbar Then menu hamburguesa despliega items", async ({
		page,
	}) => {
