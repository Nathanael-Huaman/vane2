...
test.describe("Ticket 27 - Registro y navegacion (Seguridad)", () => {
	...
	test("registro duplicado devuelve mensaje neutro sin filtrar proveedor", async ({
		page,
	}) => {
		...
		const errorText = await page
			.getByText("Ya existe una cuenta con ese correo electronico.")
			.textContent();
	...
	test("visitante no visualiza rutas de administrador en navbar", async ({
		page,
	}) => {
	...
	test("verificar-email sin token redirige a login", async ({ page }) => {
