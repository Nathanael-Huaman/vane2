...
test.describe("Ticket 14 - Cambio entre vista cliente y administrador", () => {
	...
	test("solo admins ven el selector de vista", async ({ page }) => {
		...
		await expect(page.getByText("Perfil de usuario")).toBeVisible();
		await expect(
			page.getByText("Selector de vista del administrador"),
		).toHaveCount(0);
		await expect(
			page.getByRole("button", { name: "Vista administrador" }),
		).toHaveCount(0);
	});

	test("admin cambia de vista y el modo persiste durante la sesion actual", async ({
		page,
	}) => {
	...
	test("una nueva sesion de admin reinicia la vista por defecto", async ({
		page,
	}) => {
		await seedAuthenticatedSession(page, TEST_ADMIN_EMAIL);
