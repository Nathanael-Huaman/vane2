...
test.describe("Ticket 27 - Registro y navegacion (Casos de borde)", () => {
	...
	test("doble click en crear cuenta no produce doble submit concurrente", async ({
		page,
	}) => {
	...
	test("registro normaliza correo con espacios y mayusculas", async ({
		page,
	}) => {
	...
	test("reenviar en confirmacion evita envios rapidos duplicados", async ({
		page,
	}) => {
		...
		await resend.click();
		await expect(
			page.getByText("No pudimos identificar tu correo."),
		).toBeVisible();
	});
	...
	test("navbar actualiza estado tras login de cliente", async ({ page }) => {
		...
		await page.getByLabel("Correo electrónico").fill(TEST_CLIENTE_EMAIL);
		await page
