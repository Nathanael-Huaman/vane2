...
test.describe("Ticket 10 - Resolucion de rol post-login", () => {
	...
	test("cliente autenticado ve solo experiencia de cliente", async ({
		page,
	}) => {
	...
	test("administrador autenticado ve solo experiencia de administrador", async ({
		page,
	}) => {
	...
	test("sin sesion activa, /perfil no filtra paneles protegidos", async ({
		page,
	}) => {
		await page.goto("/perfil");
	...
	test("credenciales invalidas muestran error minimo y no navegan a /perfil", async ({
		page,
	}) => {
