...
test.describe("Ticket 11 - Redireccionamiento y entrada a la tienda", () => {
	...
	test("cliente por credenciales redirige a /tienda", async ({ page }) => {
	...
	test("acceso directo a /tienda sin sesion muestra catalogo publico", async ({
		page,
	}) => {
		await page.goto("/tienda");

		await expect(page.getByText("Tienda Obstedesign")).toBeVisible();
		await expect(page.getByText("Sesion no iniciada")).toHaveCount(0);
		await expect(page.getByText("Opciones extra de administrador")).toHaveCount(
			0,
		);
	});

	test("sesion persistida admin entra a tienda sin pasar por formulario", async ({
		page,
	}) => {
